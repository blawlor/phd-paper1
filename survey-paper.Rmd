...
```{r fig.width=7.5, fig.height=4.5, echo=FALSE, warning=FALSE, fig.cap="Key Components of Software Engineering"}
library(png)
library(grid)
...
We conducted two parallel surveys, one distributed to life scientists, and the other to developers of business software. In both cases we asked questions to identify attitudes towards certain key 'markers' of software engineering as described in the previous section. We reached 81 life scientists, 45 of whom developed their own software, and 31 business software developers. We used the Likert system of questionnaire in which respondents rate their attitudes to statements from *strongly disagree* to *strongly agree*, with a total of 5 degrees to choose from. We present the results below in a form that most usefully compares the difference between the two groups, i.e. probability density graphs that treat the answers as if they were continuous variables. The purpose of the business software data is to act as a control for attitudes towards the software engineering 'markers'. Life scientist responses are in red, business software developer responses are in blue.^[Note that we used the R language to clean and analyse data. The data, code and the source markdown for this paper can be examined at https://github.com/blawlor/phd-paper1.git. The survey was carried out using surveymonkey and the data was exported in anonymised csv format.]
...
```{r infrastructure, echo = FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.width=16, fig.height=12.5, fig.cap="Responses to questions on infrastructure"}
...
plot(processes, type="density", legend = "Profession", centered="false", legend.position = "bottom") + ggtitle(title) + titleTheme + textTheme

```

When it comes to processes (automated or automatable) applied using the elements of infrastructure, the distinction between life scientists and the control group is still clear even if less pronounced. This difference is mostly a function of a reduced consensus among software engineers rather than a positive change in attitudes from the life scientists. A particular point to notice is that although there is a relatively good showing for the use of source control in the previous set of results, life scientists generally *neither agree nor disagree* with the use of branching, despite the fact branching is one of the main advantages of using source control. 

```{r practices, echo = FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.width=16, fig.height=20, fig.cap="Responses to questions on practices"}
...
plot(practices, type="density", legend = "Profession", centered="false", legend.position = "bottom") + ggtitle(title) + titleTheme + textTheme

```

As we look at the results for practices and skills, a pattern begins to emerge. The further up the pyramid we go, the 'softer' the consensus among software engineers, while the attitudes of the life scientists remain more or less static. The overall picture of a clean, albeit smaller, separation remains. 

```{r goals, echo = FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.width=16, fig.height=12.5, fig.cap="Responses to questions on goals"}
...
```{r fig.width=6, fig.height=3.5, echo=FALSE, warning=FALSE, fig.cap="Suggested project Roles of bioinformaticians and  bioinformatic engineers."}
library(png)
library(grid)
...
```{r fig.width=7, fig.height=3.5, echo=FALSE, warning=FALSE, fig.cap="Handover points between bioinformatican and bioinformatic engineers."}
library(png)
library(grid)
